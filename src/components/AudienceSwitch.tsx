...
export function AudienceSwitch() {
  ...
  // This function handles the logic that was previously in scripts.js
  const handleSwitch = (id: string) => {
    setActive(id);
    
    // Update the DOM to show/hide the correct sections
  ...
  return (
    <div className="relative mx-auto flex w-fit items-center rounded-full bg-slate-100/80 p-1.5 shadow-inner backdrop-blur-md border border-slate-200/50 mb-12">
      {options.map((option) => (
        <button
          key={option.id}
